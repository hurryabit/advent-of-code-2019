...
enum Direction {
    ...
    func delta() -> Point {
        switch self {
        case .L: return Point(row: 0, col: -1)
        case .R: return Point(row: 0, col: 1)
        case .D: return Point(row: 1, col: 0)
        case .U: return Point(row: -1, col: 0)
        }
    }
}
...
struct Segment {
    ...
    static func parse(_ s: String.SubSequence) throws -> Segment {
        if let c = s.first {
            let direction = try Direction.parse(c)
            if let length = Int(s.dropFirst()) {
                return Segment(direction: direction, length: length)
            } else{
...
struct Wire {
    ...
    static func parse(_ s: String) throws -> Wire {
        Wire(segments: try s.split(separator: ",").map(Segment.parse))
    }

    func forEach(fn: (Point, Int) -> Void) {
        var point = Point.origin
        var dist = 0
        for segment in self.segments {
            let delta = segment.direction.delta()
...
let instructions1 = try Wire.parse(input[0])
let instructions2 = try Wire.parse(input[1])

var dists1 = [Point: Int]()
instructions1.forEach {
    if !dists1.keys.contains($0) { dists1[$0] = $1 }
}

var minDist = 1000000000
instructions2.forEach {
    if let dist1 = dists1[$0] {
        let dist = dist1 + $1
        if dist < minDist {
            minDist = dist
        }
