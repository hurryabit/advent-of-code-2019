...
enum Direction {
    ...
    static func parse(_ c: Character) throws -> Direction {
        switch c {
        case "L": return .L
        case "R": return .R
        case "D": return .D
        case "U": return .U
        default: throw ParseError(msg: "Invalid direction: \(c)")
        }
    }
}
...
struct Instruction {
    ...
    static func parse(_ s: String.SubSequence) throws -> Instruction {
        if let c = s.first {
            let direction = try Direction.parse(c)
            if let length = Int(s.dropFirst()) {
                return Instruction(direction: direction, length: length)
            } else{
                throw ParseError(msg: "Invalid length: \(s.dropFirst())")
            }
        } else {
            throw ParseError(msg: "Empty instruction")
        }
    }

    static func parseMany(_ s: String) throws -> [Instruction] {
        try s.split(separator: ",").map(parse)
    }
}
...
struct Point {
    ...
    static let origin = Point(row: 0, col: 0)
...
enum Segment {
    case horizontal(row: Int, startCol: Int, endCol: Int)
    case vertical(col: Int, startRow: Int, endRow: Int)

    func intersection(_ other: Segment) -> Point? {
        switch (self, other) {
        case (.horizontal(let row, let startCol, let endCol), .vertical(let col, let startRow, let endRow)):
            if startRow <= row && row <= endRow && startCol <= col && col <= endCol {
                return Point(row: row, col: col)
            } else {
                return Optional.none
            }
        case (.vertical(let col, let startRow, let endRow), .horizontal(let row, let startCol, let endCol)):
            if startRow <= row && row <= endRow && startCol <= col && col <= endCol {
                return Point(row: row, col: col)
            } else {
                return Optional.none
    ...
    static func fromInstructions(_ instructions: [Instruction]) -> [Segment] {
        var location = Point(row: 0, col: 0)
        return instructions.map{(instruction) -> Segment in
            switch instruction.direction {
            case .L:
                let endCol = location.col - 1
                location.col -= instruction.length
                return .horizontal(row: location.row, startCol: location.col, endCol: endCol)
            case .R:
                let startCol = location.col + 1
                location.col += instruction.length
                return .horizontal(row: location.row, startCol: startCol, endCol: location.col)
            case .D:
                let startRow = location.row + 1
                location.row += instruction.length
                return .vertical(col: location.col, startRow: startRow, endRow: location.row)
            case .U:
                let endRow = location.row - 1
                location.row -= instruction.length
                return .vertical(col: location.col, startRow: location.row, endRow: endRow)
            }
        }
    }
}

let segments1 = Segment.fromInstructions(try Instruction.parseMany(input[0]))
let segments2 = Segment.fromInstructions(try Instruction.parseMany(input[1]))

var minimalDistance = 1000000000

for segment1 in segments1 {
    for segment2 in segments2 {
        if let intersection = segment1.intersection(segment2) {
            let distance = intersection.dist(Point.origin)
